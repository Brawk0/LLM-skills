...
- **ModelSim requires a variable to be declared before the task that reads it; Verilator does not.** A `int lat` placed below the task that uses it compiles clean under Verilator and stops ModelSim with `(vlog-2730) Undefined variable`. Same family as the port-connection case below: declaration order is invisible in the fast simulator and fatal in the slow one, so it surfaces long after the edit.
