...
Дальше нужен не RTL, а размещение: загнать схему в область кристалла. В Gowin
это `GROUP` и `GRP_LOC` в .cst, но имя экземпляра там нужно ПОСЛЕ уплощения
иерархии — `obj_encoder_processing` инструмент не находит, а синтаксис области
подбором не взялся. Задача отдельная и инструментальная.
